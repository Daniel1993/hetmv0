 ...
 */
#ifndef USE_TSX_IMPL
#define TM_START(tid, ro)               { stm_tx_attr_t _a = {{.id = tid, .read_only = ro}}; sigjmp_buf _tmp; sigjmp_buf *_e = stm_start(_a, &_tmp); if (_e != NULL) sigsetjmp(*_e, 0)
...
#define TM_INIT               HTM_init(MAX_NB_THREADS)
#define TM_EXIT               HTM_exit()
#define TM_INIT_THREAD        HTM_thr_init(); HTM_reset_status_count(); HTM_set_is_record(1)
#define TM_EXIT_THREAD        HTM_thr_exit()
#endif /* USE_TSX_IMPL */
...
#endif /* ! TM_COMPILER && ! USE_TSX_IMPL */
#ifdef USE_TSX_IMPL
  // replace the aborts with TSX aborts
  d->nb_aborts = HTM_get_status_count(HTM_ABORT, NULL);
#endif
  /* Free transaction */
  TM_EXIT_THREAD;
...
int main(int argc, char **argv)
{
  ...
  /*Save to file*/
  FILE *statsFile = fopen("BankStats.txt", "a");
  if (ftell(statsFile) < 8) {
		fprintf(statsFile, "sep;\n"
			"nb_accounts(1);"
			"nb_threads(2);"
			"throughput(3);"
			"prob_abort(4)\n"
  ...
  fprintf(statsFile, "%i;%i;%f;%f\n", nb_accounts, nb_threads,
    (reads + writes + updates) * 1000.0 / duration,
    (double)aborts / (double)(aborts + reads + writes + updates));
  fclose(statsFile);
