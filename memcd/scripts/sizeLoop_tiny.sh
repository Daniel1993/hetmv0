...
if [ "$BASELINE" == 1 ]; then
	#GEN CPU
	make clean
	nvcc pr-stm.cu -I tiny1/include -I tiny1/src -c $flagsGPU -DgenOnce=1
	nvcc bank.c -lstm -lpthread -I tiny1/include -I tiny1/src -c -DCPUEn=1 -DGPUEn=0 $flagsCPU
	make
	...
	make clean
	nvcc pr-stm.cu -I tiny2/include -I tiny2/src -c $flagsGPU
	nvcc bank.c -lstm -lpthread -I tiny2/include -I tiny2/src -c -DCPUEn=0 -DGPUEn=1 $flagsCPU
	make
	#GEN GPU
	for ((  i = $start  ;  i <= $end;  i+=$step ))
	do
		./bank -n $threads -a $i  -r 0 -w 0 -f  $filename'-GPU-L' -i $iter -d 2000 $runflags
	done
	for ((  i = $start2  ;  i <= $end2;  i+=$step2  ))
	do
		make
		./bank -n $threads -a $i  -r 0 -w 0 -f  $filename'-GPU-H' -i $iter -d 2000 $runflags
	done
fi
...
do
	...
	if [ "$RUN0" == 1 ]; then
		make clean
		nvcc pr-stm.cu $tmincludes -c $flagsGPU -DgenOnce=1
		nvcc bank.c -c -DCPUEn=1 -DGPUEn=1 -lstm -lpthread $tmincludes -DCuda_Confl=0 $flagsCPU
		make ROOT=$root
