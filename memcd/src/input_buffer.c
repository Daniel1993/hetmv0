...
void GPUbuffer_ZIPF_2()
{
  ...
  unsigned rnd; // = (*zipf_dist)(generator);
	for (int i = 0; i < buffer_last; ++i) {
		rnd = (zipf_gen() % parsedData.CONFL_SPACE) + parsedData.CONFL_SPACE;
		cpu_ptr[i] = rnd;
	}
	...
	for (int i = 0; i < buffer_last; ++i) {
    rnd = zipf_gen() % parsedData.CONFL_SPACE; // different from NO_CONFL --> fills the buffer
		cpu_ptr[i] = rnd;
	}
	...
	for (int i = 0; i < buffer_last; ++i) {
    rnd = zipf_gen() % parsedData.CONFL_SPACE;
		cpu_ptr[i] = rnd;
	}
}
