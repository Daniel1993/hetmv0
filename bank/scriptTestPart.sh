	...
	do
		...
		do
			...
			do
				...
				do
					# TSX
					make clean ; make GPU_PART=$g CPU_PART=$c P_INTERSECT=$i USE_TSX_IMPL=1 -j 14
					./bank -n $THREADS -a $d -r 0 -w 0 -d $DURATION
				done
				...
				do
					# Tiny
					make clean ; make GPU_PART=$g CPU_PART=$c P_INTERSECT=$i USE_TSX_IMPL=0 -j 14
					./bank -n $THREADS -a $d -r 0 -w 0 -d $DURATION
				done
