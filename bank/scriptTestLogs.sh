...
do
	...
	do
		# TSX
		make clean ; make CMP_TYPE=COMPRESSED GPU_PART=$GPU_PART CPU_PART=$CPU_PART P_INTERSECT=$P_INTERSECT USE_TSX_IMPL=1 -j 14
		for t in $THREADS
		do
		...
		# TSX
		make clean ; make CMP_TYPE=EXPLICIT GPU_PART=$GPU_PART CPU_PART=$CPU_PART P_INTERSECT=$P_INTERSECT USE_TSX_IMPL=1 -j 14
		for t in $THREADS
		do
			./bank -n $THREADS -a $DATASET -b $BLOCKS -x $GPU_THREADS -d $DURATION -T $b
		done
		...
		# Tiny
		make clean ; make CMP_TYPE=COMPRESSED GPU_PART=$GPU_PART CPU_PART=$CPU_PART P_INTERSECT=$P_INTERSECT USE_TSX_IMPL=0 -j 14
		for t in $THREADS
		do
		...
		# Tiny
		make clean ; make CMP_TYPE=EXPLICIT GPU_PART=$GPU_PART CPU_PART=$CPU_PART P_INTERSECT=$P_INTERSECT USE_TSX_IMPL=0 -j 14
		for t in $THREADS
		do
			./bank -n $t -a $DATASET -b $BLOCKS -x $GPU_THREADS -d $DURATION -T $b
		done
