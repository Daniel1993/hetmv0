...
runtext[4]='-VBAL'
runtext[5]='-VI'
includes[0]='-I tiny0/include -I tiny0/src -UCPU_INV'
includes[1]='-I tiny1/include -I tiny1/src -UCPU_INV'
...
if [ "$BASELINE" == 1 ]; then
	...
	do
		...
		do
			make clean
			nvcc pr-stm.cu -lstm -lpthread -I ../tinystm/include -I ../tinystm/src -c -DDEFAULT_TransEachThread=$i $flagsGPU
			nvcc bank.c -lstm -lpthread -I ../tinystm/include -I ../tinystm/src -c -DDEFAULT_TransEachThread=$i -DGPUEn=0 $flagsCPU
			make
		...
		make clean
		nvcc pr-stm.cu $tmincludes -lpthread -c $flagsGPU
		nvcc bank.c $tmincludes -lpthread -c -DCuda_Confl=0 $flagsCPU $flagsGPU -DCPUEn=0
		make ROOT=tiny2
...
do
	...
	do
		...
		make clean
		nvcc pr-stm.cu $tmincludes -lstm -lpthread -c $flagsGPU
		nvcc bank.c $tmincludes -lstm -lpthread -c -DCuda_Confl=0 $flagsCPU
		make ROOT=$root
		...
		if [ "$RUN5" == 1 ]; then
			#GEN LC
			rm -f  bank bank.o
			nvcc bank.c $tmincludes -lstm -lpthread -c $flagsCPU $flags5
			make ROOT=$root
		...
		if [ "$RUN25" == 1 ]; then
			...
			rm -f  bank bank.o
			nvcc bank.c $tmincludes -lstm -lpthread -c $flagsCPU $flags25
			make ROOT=$root
		...
		if [ "$RUN50" == 1 ]; then
			...
			rm -f  bank bank.o
			nvcc bank.c $tmincludes -lstm -lpthread -c $flags50 $flagsCPU
			make ROOT=$root
		...
		if [ "$RUN100" == 1 ]; then
			...
			rm -f  bank bank.o
			nvcc bank.c $tmincludes -lstm -lpthread -c $flags100 $flagsCPU
			make ROOT=$root
...
if [ "$VERBALLOOP" == 1 ]; then
	...
	if [ "$RUN50" == 1 ]; then
		...
		do
			...
			do
				rm -f  bank bank.o
				nvcc bank.c -lstm -lpthread -I tiny2/include -I tiny2/src -c -DCPUEn=1 -DGPUEn=1 $flagsCPU -DSYNC_BALANCING_VALF=10 -DSYNC_BALANCING_VALS=$i -DLABEL=$i $flags50
				make
	...
	if [ "$RUN50" == 1 ]; then
		...
		do
			...
			do
				rm -f  bank bank.o
				nvcc bank.c -lstm -lpthread -I tiny2/include -I tiny2/src -c -DCPUEn=1 -DGPUEn=1 $flagsCPU -DSYNC_BALANCING_VALS=$i -DSYNC_BALANCING_VALS=$i -DLABEL=$i $flags100
				make
			...
			do
				rm -f  bank bank.o
				nvcc bank.c -lstm -lpthread -I tiny2/include -I tiny2/src -c -DCPUEn=1 -DGPUEn=1 $flagsCPU -DSYNC_BALANCING_VALF=$i -DSYNC_BALANCING_VALS=$i -DLABEL=$i $flags100
				make
				./bank -n $threads -a $acct  -r 0 -w 0 -f $filename2'-VBAL'$i'-100I' -i $iter -d $duration -T $k $runflags
			done
...
# do
	# make clean
	# nvcc pr-stm.cu $tmincludes -lstm -lpthread -I ../tinystm/include -I ../tinystm/src -c -DBANK_NB_TRANSFERS=$i -DfirstX=1
	# make
...
# do
	# make clean
	# nvcc pr-stm.cu $tmincludes -lstm -lpthread -I ../tinystm/include -I ../tinystm/src -c -DBANK_NB_TRANSFERS=$i -DfirstX=5
	# make
...
# do
	# make clean
	# nvcc pr-stm.cu $tmincludes -lstm -lpthread -I ../tinystm/include -I ../tinystm/src -c -DBANK_NB_TRANSFERS=$i -DfirstX=7
	# make
