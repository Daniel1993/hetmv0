...
{
  return 1;
  /*tarlovskyy smashes all caches.*/
  /*1MB - 2 << 16*/
  /*2MB - 2 << 17*/
  /*4MB - 2 << 18*/
  /*8MB - 2 << 19*/
  /*long smash_n_bytes = 2 << 17;
  stm_word_t* stm_fill_cache = (stm_word_t*) malloc( smash_n_bytes * sizeof(stm_word_t) );
  if(stm_fill_cache == NULL){
    printf("Could not allocate so much memory (%d) bytes\n", smash_n_bytes);
  }
  //printf("size of stm_word_t %d, reserving to fill L3 %d bytes\n", sizeof(stm_word_t), smash_n_bytes * sizeof(stm_word_t));
  memset(stm_fill_cache, 1, smash_n_bytes * sizeof(stm_word_t));*/
  ...
  //__asm__ __volatile__("":::"memory");
    ...
    //printf("==> stm_wbetl_validate(%p[n_entries:%d])\n", tx, tx->r_set.nb_entries);
...
# endif /* UNIT_TX */
              ...
              bench_time += time_diff(time1,time2);
              time_idx++;*/
              return 0;
          }
          /* We own the lock: OK */
...
# endif /* ! IRREVOCABLE_IMPROVED */
  ...
  //tarlovskyy added for bench ALWAYS VALIDATE AT COMMIT
